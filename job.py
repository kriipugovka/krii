...
def job():
    c = randint(1, 3)
    if c == 1:
        print('На удивление Вы приехали вовремя, несмотря на пробки, и получили одобрительный взгляд от начальника. А это многое стоит.')
    if c == 2:
        print('Вы опаздали на работу и получили выговор от начальника. В следущий раз стоит выехать пораньше.')
    if c == 3:
        print('Вы так спешили на работу, что в какой-то момент потеряли управление и врезались в столб. Но есть и плюс! Страховка, которую вы берете каждый год, наконец-то окупилась.')
running = True
car_count = 0
car_num = None
chosen_car = None
...
while running:
    if car_count == 0:
        ...
        if car_num == None:
            ...
            print('Для того чтобы отправиться в путь введите "На работу"')
            inp = input()
            if inp == 'Новая машина':
                print('Название:')
                inp = input()
